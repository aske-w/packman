...
const checkMetaAchievements = (gameResults: AchievementLocalstorage[]): Badges[] => {
  ...
  const hasPlayedAllGameModes =
    gameResults.some(x => x.gamemode == Gamemodes.BIN_PACKING) && gameResults.some(x => x.gamemode == Gamemodes.STRIP_PACKING);
  if (hasPlayedAllGameModes) res.push(Badges.PLAYED_ALL_GAME_MODES);

  // LEARNING_THE_ROPES
  const hasLearningTheRopes = ALL_GAMEMODES.every(g => wonAllAlgosInGamemodeAtLevel(gameResults, g, Levels.BEGINNER));
  if (hasLearningTheRopes) res.push(Badges.LEARNING_THE_ROPES);

  // LOOK_MA_NO_HANDS
  const has_LOOK_MA_NO_HANDS = ALL_GAMEMODES.every(g => wonAllAlgosInGamemodeAtLevel(gameResults, g, Levels.NOVICE));
  if (has_LOOK_MA_NO_HANDS) res.push(Badges.LOOK_MA_NO_HANDS);

  // CERTIFIED_EXPERT
  const has_CERTIFIED_EXPERT = ALL_GAMEMODES.every(g => wonAllAlgosInGamemodeAtLevel(gameResults, g, Levels.NOVICE));
  if (has_CERTIFIED_EXPERT) res.push(Badges.CERTIFIED_EXPERT);
...
const checkPackingAchievements = (gameResults: AchievementLocalstorage[], gamemode: Gamemodes): Badges[] => {
  ...
  switch (gamemode) {
    case Gamemodes.STRIP_PACKING:
      if (has_COMPETED_AGAINST_ALL_ALGORITHMS) res.push(Badges.COMPETED_AGAINST_ALL_ALGORITHMS_STRIP_PACKING);
      if (has_WON_AGAINST_ANY_ALGORITHM) res.push(Badges.WON_AGAINST_ANY_ALGORITHM_STRIP_PACKING);
      if (has_WON_AGAINST_ALL_ALGORITHM) res.push(Badges.WON_AGAINST_ALL_ALGORITHM_STRIP_PACKING);
      if (has_LOSE_AGAINST_ANY_ALGORITHM) res.push(Badges.LOSE_AGAINST_ANY_ALGORITHM_STRIP_PACKING);
      if (has_LOSE_AGAINST_ALL_ALGORITHM) res.push(Badges.LOSE_AGAINST_ALL_ALGORITHM_STRIP_PACKING);
      if (has_COMPLETE_ALL_ALGORITHMS_BEGINNER) res.push(Badges.COMPLETE_ALL_ALGORITHMS_BEGINNER_STRIP_PACKING);
      if (has_COMPLETE_ALL_ALGORITHMS_NOVICE) res.push(Badges.COMPLETE_ALL_ALGORITHMS_INTERMEDIATE_STRIP_PACKING);
      if (has_COMPLETE_ALL_ALGORITHMS_EXPERT) res.push(Badges.COMPLETE_ALL_ALGORITHMS_EXPERT_STRIP_PACKING);
      break;
    case Gamemodes.BIN_PACKING:
      if (has_COMPETED_AGAINST_ALL_ALGORITHMS) res.push(Badges.COMPETED_AGAINST_ALL_ALGORITHMS_BIN_PACKING);
      if (has_WON_AGAINST_ANY_ALGORITHM) res.push(Badges.WON_AGAINST_ANY_ALGORITHM_BIN_PACKING);
      if (has_WON_AGAINST_ALL_ALGORITHM) res.push(Badges.WON_AGAINST_ALL_ALGORITHM_BIN_PACKING);
      if (has_LOSE_AGAINST_ANY_ALGORITHM) res.push(Badges.LOSE_AGAINST_ANY_ALGORITHM_BIN_PACKING);
      if (has_LOSE_AGAINST_ALL_ALGORITHM) res.push(Badges.LOSE_AGAINST_ALL_ALGORITHM_BIN_PACKING);
      if (has_COMPLETE_ALL_ALGORITHMS_BEGINNER) res.push(Badges.COMPLETE_ALL_ALGORITHMS_BEGINNER_BIN_PACKING);
      if (has_COMPLETE_ALL_ALGORITHMS_NOVICE) res.push(Badges.COMPLETE_ALL_ALGORITHMS_INTERMEDIATE_BIN_PACKING);
      if (has_COMPLETE_ALL_ALGORITHMS_EXPERT) res.push(Badges.COMPLETE_ALL_ALGORITHMS_EXPERT_BIN_PACKING);
      break;
  }
