...
const InputDesignerModal: React.FC<InputDesignerModalProps> = ({ existingRects, setExistingRects, visible, onClose, maxWidth, maxHeight }) => {
  ...
  useEffect(() => {
    if (visible) setNewDimensions(existingRects);
    else setNewDimensions([]);
  ...
  const onMouseDown = (e: KonvaEventObject<MouseEvent>) => {
    ...
    if (rect != undefined) {
      ...
      if (rect.x < pointerPos.x && pointerPos.x < x2 && rect.y < pointerPos.y && pointerPos.y < y2) return;
    }
  ...
  const onMouseMove = (e: KonvaEventObject<MouseEvent>) => {
    ...
    if (maxWidth) width = clamp(scaledWidth, -maxWidth, maxWidth);
    if (maxHeight) height = clamp(scaledHeight, -maxHeight, maxHeight);
  ...
  useEffect(() => {
    if (stage != null) setStageWidth(stage.clientWidth);
  }, [stage?.clientWidth]);
  ...
  const onCreateRandom = () => {
    ...
    if (parsedNumber !== parsedNumber) {
      // true if genNum couldn't be parsed
    ...
    if (maxHeight) if (max) max = maxHeight < max ? maxHeight : max;
              ...
              <div className="inline-block w-full max-w-4xl p-6 my-8 overflow-hidden prose text-left align-middle transition-all transform bg-zinc-600 shadow-xl rounded-2xl h-[40rem] overflow-y-hidden">
                <div className="flex justify-between h-[38rem]">
                  <div className="w-2/5 h-full overflow-y-scroll">
                    <div className="ml-4">
                      ...
                      <div className="flex justify-right mt-2">
                        <button
                          ...
                          className={`inline-flex justify-center mr-2 px-4 py-2 text-sm font-medium text-blue-900 bg-blue-100 border border-transparent rounded-md  focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-blue-500 ${
                            !visible ? 'opacity-60' : 'hover:bg-blue-200'
                          }`}
                          disabled={!visible}
                        >
                          Generate N
                    ...
                    <BoxInput
                      dimensionsStorage={newDimensions}
                      setDimensionsStorage={setNewDimensions}
                  ...
                  <div className="w-3/5 text-white" ref={interactiveRef}>
                    ...
                    <div className="grid grid-cols-12 mb-3">
                      <div className="col-span-6">
                        Rect:{' '}
                        {rect
                          ? 'Width: ' + Math.abs(getScaledValue(rect!.width)) + ', height: ' + Math.abs(getScaledValue(rect!.height))
                          : 'None yet'}
                      </div>
                    ...
                    <Stage
                      height={300}
                      width={stageWidth}
                      className="bg-gray-200 max-w-full"
                      onMouseDown={onMouseDown}
                      onMouseUp={onMouseUp}
                      onMouseMove={e => onMouseMove(e)}
                    >
                      <Layer fill={'#333'}>
                        <Rect {...rect} draggable onDragEnd={handleDragEnd} onDragMove={handleDragMove}></Rect>
                      </Layer>
                    </Stage>
                  </div>
                </div>
