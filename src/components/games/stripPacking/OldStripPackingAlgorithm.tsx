...
const OldStripPackingAlgorithm = React.forwardRef<StripPackingAlgorithmCanvasHandle, StripPackingAlgorithmCanvasProps>(
  ({ input, algorithm, scrollableStripHeight }, ref) => {
    const [stripRects, setStripRects] = useState<ColorRect<RectangleConfig & PrevPos>[]>([]);
    const [inventoryRects, setInventoryRects] = useState<ColorRect<RectangleConfig>[]>([]);
    ...
    const calcInitialPositions = (rects: DimensionsWithConfig<RectangleConfig>[]) => {
    ...
    const totalHeight = useMemo(() => {
      return stripRects.reduce((maxY, { y }) => Math.max(maxY, Math.round(scrollableStripHeight - y - GAME_HEIGHT)), 0);
    }, [stripRects]);
          ...
          <Stage onWheel={handleWheel} {...STAGE_SIZE}>
            ...
            <Layer ref={inventoryLayer}>
              {inventoryRects.map(r => {
                return <Rect key={r.name} {...r} strokeWidth={2} stroke={'#002050FF'} y={GAME_HEIGHT + r.y} id={`INVENTORY_RECT`} />;
              })}
            </Layer>
          </Stage>
        </div>
...
const MyRect: React.FC<PrevPos & RectConfig & KonvaNodeEvents> = ({ x, y, prevX, prevY, ...props }) => {
  ...
  return <Rect ref={ref} x={prevX} y={prevY + GAME_HEIGHT} stroke={'rgba(0,0,0,0.2)'} strokeWidth={1} {...props}></Rect>;
};
